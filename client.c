...
int main(int argc,char* argv[]){
  ...
  for(i=0;i<5;i++)
    finfo.point[i]=0;
  int current_point=5; //point when player i out game
  int my_point=0;
#endif
  // 乱数の初期化
  ...
  while(whole_gameend_flag==0){
	...
	myCards = setBit(own_cards);
	// 出すカードを決定する
	TDLearning(select_cards, myCards, oppCards, &cinfo, &finfo);
	...
	int roop_dumy=0;
	int roop_count=0;
	for(roop_dumy=0;roop_dumy<10000000;roop_dumy++){
	  roop_count++;
	}
	genrand_int32();
      }
...
#ifdef USE_POINT_OUT
  printf("%d %d %d %d %d \n",finfo.point[0],finfo.point[1],finfo.point[2],finfo.point[3],finfo.point[4]);
#endif
