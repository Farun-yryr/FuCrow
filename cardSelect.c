/* cardSelect.c  : TD Learningで提出するカードを決定する */ 
/* First  Author : Fumiya Suto                        */
/* Second Author : Satoshi Konuma                     */
#include <math.h>
...
double dlog(int j){
  static double ret[SIMULATION_COUNT]={0.0};
...
/*
void temporaldifference(int result[5], 
			const int64 cards[5], const fieldInfo *finfo, 
			int currentPlayer,
...
void doSimulation(int result[5], 
		  ...
		  const bitValidHandsArray vhG[5],
		  const bitValidHandsArray vhS[5],
		  int targetPlayer ,double *td_score, int *td_depth){
  int i, j;
  int64 playersCard[5];
  bitValidHandsArray vhb, vhcG[5], vhcS[5];
  fieldInfo simField = *finfo;
  ...
  while(1){
    //get value only own tern
    if(current==finfo->mypos){	
      int k;
      double alpha=TD_PARAMETER;
      double count=0.0;
      //DAIHINMIN's value function(the number of lest is true value?)
      for(k=0;k<5;k++){
	if(simField.lest[k]>0){	  
	  count=count+1.0;
	}
      }
      if(*td_score==0.0){
	...
	*td_score=*td_score*(1.0-alpha)+count*alpha;
      }
    }
...
// 現在の状況からTD Learningで行動を決定する
// 事前に必勝手探索なんかも行なっている
void TDLearning(int out_cards[8][15], int64 myCards, int64 oppCards, const changeInfo *cinfo, const fieldInfo *finfo){
  int i, j;
  // 各プレイヤーの手札
  int64 playersCard[5];
  // 各手のシミュレーション回数
  ...
  if(decFlag == -1){
	...
	if(simulateCount[j] == 0) 
	  ...
	  double V = avg2-(avg*avg)+sqrt((2*dlog(i))/simulateCount[j]);
	  if(V > 0.25) V = 0.25;
	  score[j] = avg + sqrt(V*log((double)i)/simulateCount[j]);
	  
	  /*
	  double avg  = simulateScore[j];
	  double avg2 = simulateDScore[j];
	  double V = avg2 -(avg *avg/simulateCount[j])+(2*dlog(i));
	  if(V > 0.25) V = 0.25;
	  score[j] = (avg + V*dlog(i))/simulateCount[j];
	  */
	}
      }
      int idx = 0;
	...
	if(j==finfo->mypos){
      ...
      doSimulation(resultRank, playersCard, &simField, current, vhcG, vhcS, finfo->mypos ,&td_score , &td_depth);

      int myRank = resultRank[finfo->mypos];
      double reward=td_score;
      double beta=TD_PARAMETER_BETA;
      if(myRank==1)
	reward=reward*(1.0-beta)+1.0*beta;
      printf("reward%f\n\n",reward);
      // シミュレートの結果を更新
      simulateCount[idx]++;
      simulateScore[idx] += (reward*reward)/25.0;
      simulateDScore[idx] += (reward*reward*reward*reward)/(25.0*25.0);
